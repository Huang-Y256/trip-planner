...
import { generateTripPlanStream, type StreamEvent } from '@/services/api'
...
// LangGraph 节点 → 用户可读文案 + 进度百分比
const NODE_CONFIG: Record<string, { label: string; progress: number }> = {
  extract_prefs:   { label: '🔍 正在分析您的偏好...',       progress: 10 },
  parallel_fetch:  { label: '📡 正在并行获取景点/天气/酒店...', progress: 30 },
  planner:         { label: '📋 AI 正在生成行程计划...',      progress: 60 },
  validator:       { label: '✅ 正在校验计划数据...',         progress: 85 },
  replanner:       { label: '🔄 校验未通过，正在修正...',      progress: 50 },
  fallback:        { label: '⚠️ 正在生成降级占位计划...',     progress: 90 },
}
...
const handleSubmit = async () => {
  ...
  loadingStatus.value = '🚀 正在启动多智能体流水线...'
  ...
  try {
    ...
    const response = await generateTripPlanStream(requestData, (event: StreamEvent) => {
      // 每收到一个 SSE 事件，更新进度条和状态文本
      const config = NODE_CONFIG[event.node]
      if (config) {
        loadingStatus.value = event.message || config.label
        loadingProgress.value = Math.max(loadingProgress.value, config.progress)
      }
      if (event.done) {
        loadingProgress.value = 100
        loadingStatus.value = event.node === 'error' ? `❌ ${event.message}` : '✅ 完成!'
      }
    ...
    if (response.success && response.data) {
      ...
      // 短暂延迟后跳转
      setTimeout(() => {
        // 通过 plan_id 跳转（后端已持久化，URL 可直接分享）
