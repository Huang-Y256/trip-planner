...
const planId = ref<string>(route.params.planId as string || '')
...
onMounted(async () => {
  if (planId.value) {
    try {
      const response = await getTripPlan(planId.value)
      if (response.success && response.data) {
        tripPlan.value = response.data
      } else {
        message.error('旅行计划不存在或已被删除')
        return
      }
    } catch (error: any) {
      message.error(error.message || '加载旅行计划失败')
      return
    }
    await nextTick()
    initMap()
    // 地图显示后，后台异步加载景点图片（不阻塞页面渲染）
...
const saveChanges = async () => {
  ...
  if (tripPlan.value && planId.value) {
    try {
      await updateTripPlan(planId.value, tripPlan.value)
...
const initMap = async () => {
  try {
    // 高德 JS API 2.0 安全密钥配置（2021年12月后创建的 Key 必须设置）
...
/* 图片加载中的 shimmer 骨架屏效果 */
.attraction-image-wrapper:has(.attraction-image:not(.loaded))::after {
  content: '';
  position: absolute;
  inset: 0;
  background: linear-gradient(110deg, #e0e0e0 30%, #f5f5f5 50%, #e0e0e0 70%);
  background-size: 200% 100%;
  animation: shimmer 1.5s infinite;
}

.attraction-image-wrapper:has(.attraction-image.loaded)::after {
  display: none;
}
